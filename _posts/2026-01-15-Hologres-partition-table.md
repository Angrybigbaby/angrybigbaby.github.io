...
我们常用的分区键是时间字段。那么为什么时间分区更适合 RANGE 而不是 LIST？首次在 Hologres 建分区表时，我在思考这个问题。

所以，以时间字段为例，我们来从几个角度分析一下。
...
FOR VALUES FROM ('2025-01-01') TO ('2025-02-01')
```
系统理解这是一个左闭右开的时间段，任何落在该区间的值都归属此分区。

LIST 分区只能接受离散值列表：
```sql
FOR VALUES IN ('2025-01-01', '2025-01-02', ..., '2025-01-31')  -- 手动列31天？
```
这不仅冗长，而且丢失了连续性语义。因为数据库不知道 '2025-01-15' 和 '2025-01-16' 是相邻的。
RANGE 表达的是 “一段时光”，LIST 表达的是 “几个日子”。
